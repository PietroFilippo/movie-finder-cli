...
- **Flexible Downloading & Streaming:**
  ...
  - **Direct Terminal Download:** Use `webtorrent-cli` or `peerflix` integration to download files directly within the terminal, with native interactive progress bars.
  - **Stream to VLC:** Stream media directly to VLC Media Player using `webtorrent-cli` or `peerflix`. Press the `v` hotkey at any time during a streaming session to reopen VLC without losing your torrent download/buffering progress.
